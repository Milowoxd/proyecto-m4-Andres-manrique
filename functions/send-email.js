...
module.exports = async function handler(req, res) {
  ...
  if (!email || !tasks) {
    return res.status(400).json({ error: 'Email y tareas son requeridos' });
  }
  ...
  const htmlBody = `
    ...
    ${pending.length > 0 ? `
      ...
      <ul>
        ${pending.map(t => `<li>${t.title}${t.description ? ` - ${t.description}` : ''}</li>`).join('')}
      </ul>
    ` : ''}
    ...
    ${completed.length > 0 ? `
      ...
      <ul>
        ${completed.map(t => `<li><s>${t.title}</s></li>`).join('')}
      </ul>
    ` : ''}
  `;
